...
int main()
{
	...
	// Чтобы в итоговом списке каждый элемент был не больше среднего среди следующих, будем идти с конца
	// Таким образом, если мы решим не удалять какой-то элемент, то для него условие дальше никогда не испортится
	...
	while (p) {
		if (count == 0 || p->inf * count <= sum) { // если элемент можно оставить, то оставляем
			sum += p->inf;
			count++;
			p = p->prev;
		}
